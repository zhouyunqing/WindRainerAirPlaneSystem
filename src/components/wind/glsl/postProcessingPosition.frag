...
vec3 generateRandomParticle(vec2 seed, float lev) {
	...
	float randomLon = mod(rand(seed, lonRange) + 180.0, 360.0) - 180.0;
...
bool particleOutbound(vec3 particle) {
    ...
    return particle.x < lonRange.x || particle.x > lonRange.y || particle.y < latRange.x || particle.y > latRange.y ;
}
